...
| 功能 | 说明 |
|------|------|
| 🔍 新作品检测 | 每小时自动检测指定用户是否发布新作品，同一时段发布多条时仅通知最新一条 |
...
**Q: 作者一次发了好多条视频，会收到一堆通知吗？**
A: 不会。检测以「当前最新作品」为时间锚点（`known_videos.json` 中发布时间/ID 最大者），
每次只推送比锚点更新的**最新 1 条**；同一批发布的其余作品会静默记录，避免刷屏。
锚点会自动推进到最新作品，下次只检测它之后的新视频。
